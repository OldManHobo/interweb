...
struct SlotMachine
{
    ...
    /*0x3F*/ u8 reelTimeAstrohautSpriteId;
    ...
    /*0x52*/ u8 reelTimeAstrohautAuraSpriteIds[2];
...
static void ReelTime_WaitStartAstrohaut(struct Task *);
static void ReelTime_AstrohautSpeedUp1(struct Task *);
static void ReelTime_AstrohautSpeedUp2(struct Task *);
...
static void ReelTime_AstrohautReact(struct Task *);
...
static void CreateReelTimeAstrohautSprite(void);
static void DestroyReelTimeAstrohautSprite(void);
...
static void CreateReelTimeAstrohautAuraSprites(void);
static void SetReelTimeAstrohautAuraFlashDelay(s16);
static void DestroyReelTimeAstrohautAuraSprites(void);
...
static void SpriteCB_ReelTimeAstrohaut(struct Sprite *);
static void SpriteCB_ReelTimeNumbers(struct Sprite *);
static void SpriteCB_ReelTimeBolt(struct Sprite *);
static void SpriteCB_ReelTimeAstrohautAura(struct Sprite *);
...
static EWRAM_DATA struct SpriteFrameImage *sImageTable_ReelTimeAstrohaut = NULL;
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimeAstrohautAura;
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimeAstrohaut;
...
static void (*const sReelTimeTasks[])(struct Task *task) =
{
    ...
    [RT_TASK_WAIT_START_PIKA]      = ReelTime_WaitStartAstrohaut,
    [RT_TASK_PIKA_SPEEDUP1]        = ReelTime_AstrohautSpeedUp1,
    [RT_TASK_PIKA_SPEEDUP2]        = ReelTime_AstrohautSpeedUp2,
    [RT_TASK_WAIT_REEL]            = ReelTime_WaitReel,
    [RT_TASK_CHECK_EXPLODE]        = ReelTime_CheckExplode,
    [RT_TASK_LAND]                 = ReelTime_LandOnOutcome,
    [RT_TASK_PIKA_REACT]           = ReelTime_AstrohautReact,
...
static const u8 sReelTimeAstrohautAnimIds[] = {1, 1, 2, 2};
static const s16 sReelTimeBoltDelays[] = {64, 48, 24, 8};
static const s16 sAstrohautAuraFlashDelays[] = {10, 8, 6, 4};
...
static bool8 SlotTask_FreeDataStructures(struct Task *task)
{
    if (!gPaletteFade.active)
    {
        ...
        TRY_FREE_AND_SET_NULL(sImageTable_ReelTimeAstrohaut);
...
static void ReelTime_WaitStartAstrohaut(struct Task *task)
{
    AdvanceReeltimeReel(task->tRtReelSpeed >> 8);
    if (++task->tTimer1 >= 60)
    {
        ...
        CreateReelTimeAstrohautAuraSprites();
    }
}

static void ReelTime_AstrohautSpeedUp1(struct Task *task)
{
    int i;
    u8 pikachuAnimIds[ARRAY_COUNT(sReelTimeAstrohautAnimIds)];
    s16 reelTimeBoltDelays[ARRAY_COUNT(sReelTimeBoltDelays)];
    s16 pikachuAuraFlashDelays[ARRAY_COUNT(sAstrohautAuraFlashDelays)];

    memcpy(pikachuAnimIds, sReelTimeAstrohautAnimIds, sizeof(sReelTimeAstrohautAnimIds));
    memcpy(reelTimeBoltDelays, sReelTimeBoltDelays, sizeof(sReelTimeBoltDelays));
    memcpy(pikachuAuraFlashDelays, sAstrohautAuraFlashDelays, sizeof(sAstrohautAuraFlashDelays));
    ...
    SetReelTimeAstrohautAuraFlashDelay(pikachuAuraFlashDelays[i]);
    StartSpriteAnimIfDifferent(&gSprites[sSlotMachine->reelTimeAstrohautSpriteId], pikachuAnimIds[i]);
...
static void ReelTime_AstrohautSpeedUp2(struct Task *task)
{
    AdvanceReeltimeReel(task->tRtReelSpeed >> 8);
    if (++task->tTimer1 >= 80)
    {
        ...
        task->tTimer1 = 0;
        SetReelTimeAstrohautAuraFlashDelay(2);
        StartSpriteAnimIfDifferent(&gSprites[sSlotMachine->reelTimeAstrohautSpriteId], 3);
    }
}
...
// Animate Astrohaut reaction. Clear any power bolts the player may have won if
// they got a positive ReelTime draw.
static void ReelTime_AstrohautReact(struct Task *task)
{
    if (++task->tTimer2 >= 60)
    {
        StopMapMusic();
        DestroyReelTimeBoltSprites();
        DestroyReelTimeAstrohautAuraSprites();
        task->tState++; // RT_TASK_WAIT_CLEAR_POWER
        if(sSlotMachine->reelTimeDraw == 0)
        {
            task->tTimer2 = 0xa0;
            StartSpriteAnimIfDifferent(&gSprites[sSlotMachine->reelTimeAstrohautSpriteId], 5);
            PlayFanfare(MUS_TOO_BAD);
        }
        else
        {
            task->tTimer2 = 0xc0;
            StartSpriteAnimIfDifferent(&gSprites[sSlotMachine->reelTimeAstrohautSpriteId], 4);
            gSprites[sSlotMachine->reelTimeAstrohautSpriteId].animCmdIndex = 0;
...
static void ReelTime_DestroySprites(struct Task *task)
{
    ...
    SetGpuReg(REG_OFFSET_BG1HOFS, 0);
    sSlotMachine->reelSpeed = REEL_NORMAL_SPEED;
    DestroyReelTimeAstrohautSprite();
...
static void CreateReelTimeAstrohautSprite(void)
{
    ...
    u8 spriteId;
    if (sImageTable_ReelTimeAstrohaut == NULL)
        sImageTable_ReelTimeAstrohaut = AllocZeroed(sizeof(struct SpriteFrameImage) * 5);

    sImageTable_ReelTimeAstrohaut[0].data = sReelTimeGfxPtr + (0 * 0x800);
    sImageTable_ReelTimeAstrohaut[0].size = 0x800;
    sImageTable_ReelTimeAstrohaut[1].data = sReelTimeGfxPtr + (1 * 0x800);
    sImageTable_ReelTimeAstrohaut[1].size = 0x800;
    sImageTable_ReelTimeAstrohaut[2].data = sReelTimeGfxPtr + (2 * 0x800);
    sImageTable_ReelTimeAstrohaut[2].size = 0x800;
    sImageTable_ReelTimeAstrohaut[3].data = sReelTimeGfxPtr + (3 * 0x800);
    sImageTable_ReelTimeAstrohaut[3].size = 0x800;
    sImageTable_ReelTimeAstrohaut[4].data = sReelTimeGfxPtr + (4 * 0x800);
    sImageTable_ReelTimeAstrohaut[4].size = 0x800;

    spriteTemplate = sSpriteTemplate_ReelTimeAstrohaut;
    spriteTemplate.images = sImageTable_ReelTimeAstrohaut;
    spriteId = CreateSprite(&spriteTemplate, 280, 80, 1);
    gSprites[spriteId].oam.priority = 1;
    gSprites[spriteId].coordOffsetEnabled = TRUE;
    sSlotMachine->reelTimeAstrohautSpriteId = spriteId;
}

static void DestroyReelTimeAstrohautSprite(void)
{
    DestroySprite(&gSprites[sSlotMachine->reelTimeAstrohautSpriteId]);
    TRY_FREE_AND_SET_NULL(sImageTable_ReelTimeAstrohaut);
}

static void SpriteCB_ReelTimeAstrohaut(struct Sprite *sprite)
{
...
static void CreateReelTimeAstrohautAuraSprites(void)
{
    ...
    u8 spriteId = CreateSprite(&sSpriteTemplate_ReelTimeAstrohautAura, 72, 80, 3);
    gSprites[spriteId].oam.priority = 1;
    ...
    sSlotMachine->reelTimeAstrohautAuraSpriteIds[0] = spriteId;

    // Right half
    spriteId = CreateSprite(&sSpriteTemplate_ReelTimeAstrohautAura, 104, 80, 3);
    gSprites[spriteId].oam.priority = 1;
    gSprites[spriteId].hFlip = TRUE;
    sSlotMachine->reelTimeAstrohautAuraSpriteIds[1] = spriteId;
}

static void SpriteCB_ReelTimeAstrohautAura(struct Sprite *sprite)
{
    u8 colors[] = {16, 0};
...
static void SetReelTimeAstrohautAuraFlashDelay(s16 delay)
{
    gSprites[sSlotMachine->reelTimeAstrohautAuraSpriteIds[0]].sDelay = delay;
}
...
static void DestroyReelTimeAstrohautAuraSprites(void)
{
    ...
    for (i = 0; i < ARRAY_COUNT(sSlotMachine->reelTimeAstrohautAuraSpriteIds); i++)
        DestroySprite(&gSprites[sSlotMachine->reelTimeAstrohautAuraSpriteIds[i]]);
}
...
// The "confusion" ducks that circle Astrohaut if the Reel Time machine explodes
static void CreateReelTimeDuckSprites(void)
{
    u8 i;
...
static const struct SpriteFrameImage sImageTable_ReelTimeAstrohautAura[] = { gSlotMachineReelTimePikaAura, 0x400 };
...
static const union AnimCmd sAnim_ReelTimeAstrohaut_Still[] =
{
    ANIMCMD_FRAME(0, 16),
    ANIMCMD_END
};

static const union AnimCmd sAnim_ReelTimeAstrohaut_ChargingSlow[] =
{
    ANIMCMD_FRAME(1, 16),
    ANIMCMD_FRAME(0, 16),
    ANIMCMD_JUMP(0)
...
static const union AnimCmd sAnim_ReelTimeAstrohaut_ChargingMedium[] =
{
    ANIMCMD_FRAME(1, 8),
    ANIMCMD_FRAME(0, 8),
    ANIMCMD_JUMP(0)
...
static const union AnimCmd sAnim_ReelTimeAstrohaut_ChargingFast[] =
{
    ANIMCMD_FRAME(1, 4),
    ANIMCMD_FRAME(0, 4),
    ANIMCMD_JUMP(0)
...
static const union AnimCmd sAnim_ReelTimeAstrohaut_Cheering[] =
{
    ANIMCMD_FRAME(2, 32),
    ANIMCMD_FRAME(3, 32),
    ANIMCMD_JUMP(0)
...
static const union AnimCmd sAnim_ReelTimeAstrohaut_FellOver[] =
{
    ANIMCMD_FRAME(4, 1),
    ANIMCMD_END
};
...
static const union AnimCmd *const sAnims_ReelTimeAstrohaut[] =
{
    sAnim_ReelTimeAstrohaut_Still,
    sAnim_ReelTimeAstrohaut_ChargingSlow,
    sAnim_ReelTimeAstrohaut_ChargingMedium,
    sAnim_ReelTimeAstrohaut_ChargingFast,
    sAnim_ReelTimeAstrohaut_Cheering,
    sAnim_ReelTimeAstrohaut_FellOver
};
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimeAstrohaut =
{
    ...
    .oam = &sOam_64x64,
    .anims = sAnims_ReelTimeAstrohaut,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCB_ReelTimeAstrohaut
};
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimeAstrohautAura =
{
    ...
    .oam = &sOam_32x64,
    .anims = sAnims_SingleFrame,
    .images = sImageTable_ReelTimeAstrohautAura,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCB_ReelTimeAstrohautAura
};
...
static const struct SpritePalette sSlotMachineSpritePalettes[] =
{
    ...
    { .data = gSlotMachineReelTimeAstrohaut_Pal,   .tag = PALTAG_REEL_TIME_ASTROHAUT},
